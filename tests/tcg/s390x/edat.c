...
static void sske(unsigned long key, unsigned long address)
{
    register unsigned long r1 asm("1") = key;
    register unsigned long r2 asm("2") = address;

    asm volatile("sske %0,%1" : "+d"(r1), "+a"(r2) : : "cc", "memory");
}

static unsigned long irbm(unsigned long address)
{
    register unsigned long r1 asm("1");
    register unsigned long r2 asm("2") = address;

    asm volatile(".insn rre,0xb9ac0000,%0,%1"
                 : "=d"(r1) : "a"(r2) : "memory");
    return r1;
}

static unsigned long rrbm(unsigned long address)
{
    register unsigned long r1 asm("1");
    register unsigned long r2 asm("2") = address;

    asm volatile(".insn rre,0xb9ae0000,%0,%1"
                 : "=d"(r1) : "a"(r2) : "memory");
    return r1;
}
...
static struct csp_result csp(uint32_t *address, uint32_t old, uint32_t new,
                             unsigned long request)
{
    register unsigned long r1 asm("1") =
        (unsigned long)address | request;
    register unsigned long r2 asm("2") = old;
    register unsigned long r3 asm("3") = new;
    unsigned long ipm;

    asm volatile("csp %%r2,%%r1; ipm %0"
                 : "=d"(ipm), "+a"(r1), "+d"(r2), "+d"(r3)
                 :
                 : "cc", "memory");
    return (struct csp_result) { r2, (ipm >> 28) & 3 };
}
...
int main(void)
{
    ...
    /*
     * IRBM reports the first block in bit 63 and the last block in bit 0.
     * Operand bits below the 256K boundary are ignored.
     */
    for (i = 0; i < 64; i++) {
        sske(0, base + i * PAGE_SIZE);
    }
    sske(0x04, base);
    sske(0x04, base + 63 * PAGE_SIZE);
    result = irbm(base + 0x2345);
    if (result != (1UL << 63 | 1)) {
        return 9;
    }
    result = rrbm(base + 0x3456);
    if (result != (1UL << 63 | 1) || irbm(base) != 0) {
        return 10;
    }

    /* CSP compares at the word-aligned R2 address and returns CC 0 or 1. */
    csp_word = 0x7ea59010;
    cr = csp(&csp_word, 0x7ea59010, 0x12345678, 1);
    if (cr.cc != 0 || cr.old != 0x7ea59010 ||
        csp_word != 0x12345678) {
        ml_printf("CSP success: cc=%x old=%x word=%x\n",
                  cr.cc, cr.old, csp_word);
        return 11;
    }
    cr = csp(&csp_word, 0x87654321, 0xabcdef01, 0);
    if (cr.cc != 1 || cr.old != 0x12345678 ||
        csp_word != 0x12345678) {
        ml_printf("CSP failure: cc=%x old=%x word=%x\n",
                  cr.cc, cr.old, csp_word);
        return 12;
    }
